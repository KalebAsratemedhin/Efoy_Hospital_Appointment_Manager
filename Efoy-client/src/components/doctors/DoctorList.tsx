...
const DoctorList = () => {
  ...
  const specialities = doctors ? [...new Set(doctors.map((doctor: Doctor) => doctor.doctorData.speciality))] as string[] : [];

  const filteredDoctors = doctors?.filter((doctor: Doctor) => {
    const matchesSearch = doctor.fullName.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         doctor.doctorData.speciality.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesSpeciality = !selectedSpeciality || doctor.doctorData.speciality === selectedSpeciality;
